const mytranslations = {
  en: {
    HomepageTitel: 'Spend me a coffee',
    MenuHome: 'Home',
    MenuProject: 'Project',
    MenuContact: 'Contact',
    ...
    ContactTitel: 'Let\s conquer the world of caffeine and creativity together!',
  ...
  de: {
    HomepageTitel: 'Lade mich zum Kaffee ein',
    MenuHome: 'Startseite',
    MenuProject: 'Projekt',
    MenuContact: 'Kontakt',
   ...
   BalanceLabel: 'Guthaben:',
    //contact translations
    ContactTitel: 'Lass uns zusammen die Welt des Koffeins und der Kreativität erobern!',
...
function setLanguage(lang) {
  ...
  for (let j = 0; j < langElements.length; j++) {
    ...
    if (mytranslations[lang] && mytranslations[lang][langKey]) {
      langElements[j].innerHTML = mytranslations[lang][langKey];
    }
  }
}
...
//Function is needet to get the text out for the animation for the Herotitle
function getHeroTitle() {
  const userLang = navigator.language || navigator.userLanguage;
  const lang = userLang === 'de' ? 'de' : 'en';
  return mytranslations[lang].HeroTitel;
}
//Function is needet to get the text out for the animation for the Contacttitle
function getContactTitle() {
  const userLang = navigator.language || navigator.userLanguage;
  const lang = userLang === 'de' ? 'de' : 'en';
  return mytranslations[lang].ContactTitel;
}
//Translate profile 
function setLanguage(lang) {
  const langElements = document.querySelectorAll('.lang');
  ...
  // Aktualisiere addressLabel und balanceLabel in spend-coffee.js
  if (addressLabel && balanceLabel) {
    addressLabel.textContent = mytranslations[lang].AdressLabel;
    balanceLabel.textContent = mytranslations[lang].BalanceLabel;
  }
}
